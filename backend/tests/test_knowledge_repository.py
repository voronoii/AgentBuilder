...
async def test_create_and_list_kb(db_session: AsyncSession) -> None:
    ...
    created = await repo.create_kb(
        KnowledgeBaseCreate(name="mydocs", description="", embedding_provider="fastembed", embedding_model="intfloat/multilingual-e5-small", embedding_dim=384, chunk_size=1000, chunk_overlap=200),
